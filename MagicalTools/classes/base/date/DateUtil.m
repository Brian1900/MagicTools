...
//  CtripTest
//
//  Created by ctrip ctrip on 12-8-22.
...
+ (NSString *)getCurrentTime
{
    ...
    return [DateUtil getCalendarStrBySimpleDateFormat:date simpleDateFormatType:SIMPLEFORMATTYPE1];
}
...
{
    NSDate *date = [NSDate date];
    return [DateUtil getCalendarStrBySimpleDateFormat:date simpleDateFormatType:SIMPLEFORMATTYPE16];
}
...
{
    NSDate *date = [NSDate date];
    return [DateUtil getCalendarStrBySimpleDateFormat:date simpleDateFormatType:SIMPLEFORMATTYPE17];
}
...
{
    NSDate *date = [NSDate date];
    return [DateUtil getCalendarStrBySimpleDateFormat:date simpleDateFormatType:SIMPLEFORMATTYPE6];
}
...
+ (NSCalendar *)getCurrentCalendar
{
    ...
    [calendar_ setTimeZone:[NSTimeZone timeZoneWithName:[DateUtil displayTimeZone]]];
...
+ (NSDate *)getDateByDateStr:(NSString *)dateStr
{
    ...
    if(dateStr.length == 8)
    {
        date = [DateUtil dateFromString:dateStr formate:@"yyyyMMdd"];
    }else if(dateStr.length == 10)
    {
        date = [DateUtil dateFromString:dateStr formate:@"yyyyMMddHH"];        
    }else if(dateStr.length == 12)
    {
        date = [DateUtil dateFromString:dateStr formate:@"yyyyMMddHHmm"];
    }else if(dateStr.length == 14)
    {
        date = [DateUtil dateFromString:dateStr formate:@"yyyyMMddHHmmss"];
    }
...
+ (NSDate *)getDateByDateTimeStr:(NSString *)dateStr
{
    ...
    while (dateStr.length < 14)
    {
     ...
     NSDate *date = [DateUtil dateFromString:dateStr formate:@"yyyyMMddHHmmss"];
...
+ (NSString *)getLastDate
{
    ...
    NSString *dateStr = [DateUtil getCalendarStrBySimpleDateFormat:date simpleDateFormatType:SIMPLEFORMATTYPE6];
...
//    date = [[[NSDate alloc] initWithTimeInterval:86400 sinceDate:date] autorelease];
    date = [DateUtil calculateDate:date field:DAY amount:1];
    NSString *dateStr = [DateUtil getCalendarStrBySimpleDateFormat:date simpleDateFormatType:SIMPLEFORMATTYPE6];
...
{
    NSCalendar *calendar = [DateUtil getCurrentCalendar];
...
+ (NSString *)getThreeDayDes:(int)count
{
    NSString *dateStr = @"";
...
+ (NSString *)getDateByStep:(NSString *)dateStr step:(int)step
{
    ...
    if(dateStr != nil && dateStr.length >=8)
    {
        ...
        }else 
        {
            ...
            date = [DateUtil getTimeByStep:dateStr field:DAY step:step];
        }
    }
...
+ (NSString *)getTimeByStep:(NSString *)dateStr field:(int)field step:(int)step
{
    ...
    NSDate *date = [DateUtil dateFromString:dateStr formate:SIMPLEFORMATTYPESTRING1];
    
    date = [DateUtil calculateCalendar:date field:field amount:step];
    
    NSString *datStr = [DateUtil dateFormateWithDate:date formate:SIMPLEFORMATTYPESTRING1];
...
+ (NSDate *)calculateCalendar:(NSDate *)date field:(int)field amount:(int)amount
{
    ...
    return [DateUtil calculateDate:date field:field amount:amount];
}
...
+ (NSString *)calendarStrBySimpleDateFormat:(NSString *)dateStr simpleDateFormatType:(int)simpleDateFormatType
{
    ...
    NSDate *date = [DateUtil getDateByDateTimeStr:dateStr];
    
    NSString *dateStr1 = [DateUtil getCalendarStrBySimpleDateFormat:date simpleDateFormatType:simpleDateFormatType];
...
//        if(dateFormater == nil)
//        {
        NSDateFormatter *dateFormater = [DateUtil getCurrentDateFormatter];
//        }
...
+ (NSDate *)dateOfString:(NSString *)dateString dateFormat:(NSString *)dateFormat
{
    ...
    if (!retDate) {
        NSTimeZone *displayTimeZone = [NSTimeZone timeZoneWithName:[DateUtil displayTimeZone]];
        NSTimeZone *storageTimeZone = [NSTimeZone timeZoneWithName:[DateUtil storageTimeZone]];
...
+ (NSString *)getShowWeekByDateString:(NSString *)dateStr
{
    ...
    NSString *dateStr1 = [DateUtil getCalendarStrBySimpleDateFormat:date simpleDateFormatType:SIMPLEFORMATTYPE10];

    NSString *weekStr = [DateUtil getShowWeekByDate:date];
...
+ (NSString *)getShowWeek:(NSString *)dateStr
{
    ...
    NSDate *startDate = [DateUtil dateFromString:dateStr formate:SIMPLEFORMATTYPESTRING6];

    NSDate *nowDate  = [NSDate date];
    int diffDay = [DateUtil compareDaysFromStartDate:startDate endDate:nowDate];
  
    // 三天之内，返回今天明天 后天
    NSString *retStr = @"";
    
    if(diffDay >= 0 && diffDay <= 2){
        retStr = [DateUtil getThreeDayDes:diffDay];
    }else {
        retStr = [DateUtil getShowWeekByDate1:startDate];
    }
...
+ (NSString *)getShowWeekStringWithDateStr:(NSString *)dateStr
{
    ...
    NSDate *nowDate  = [NSDate date];
    int diffDay = [DateUtil compareDaysWithStartDate:nowDate endDate:startDate];
...
+ (NSString *)getDateStrCompareToDay:(NSString *)dateStr
{
    ...
    NSString *today = @"";
    NSString *nowStr = [DateUtil getCurrentDate];
    NSDate *nowDate = [DateUtil dateFromString:nowStr formate:@"yyyyMMdd"];
    NSDate *date = [DateUtil dateFromString:dateStr formate:@"yyyyMMdd"];
    
    int diffDay = [DateUtil compareDaysFromStartDate:nowDate endDate:date];

//    // 三天之内，返回今天明天 后天 之外 返回 @""
    if(diffDay >= 0 && diffDay <= 2){
        today = [DateUtil getThreeDayDes:diffDay];
    }
...
+ (NSString *)getDateStrCompareWithToday:(NSString *)dateStr
{
  ...
  int diffDay = [DateUtil compareDaysWithStartDate:nowDate endDate:date];
...
+ (NSString *)getShowWeekByDate2:(NSString *)dateStr
{
    ...
    NSDate *date = [DateUtil dateFromString:dateStr formate:SIMPLEFORMATTYPESTRING6];
    
    NSString *str = [DateUtil getCalendarStrBySimpleDateFormat:date simpleDateFormatType:SIMPLEFORMATTYPE7];
    
    NSString *weekStr = [DateUtil getShowWeek:dateStr];
...
{
    NSString *holidayStr = [DateUtil getHolidayString:dateStr];
    ...
    if(holidayStr == nil || [holidayStr isEqualToString:@""])
    {
        ...
        retStr = [DateUtil getShowWeekByDate:date];
    }else
    {
        retStr = holidayStr;
    }
...
+ (NSString *)getShowWeekByDate3:(NSString *)dateStr
{
    ...
    NSString *nowDateStr = [DateUtil dateFormateWithDate:nowDate formate:@"yyyyMMdd"];
    ...
    NSString *str = [DateUtil getCalendarStrBySimpleDateFormat:startDate simpleDateFormatType:SIMPLEFORMATTYPE7];
...
+ (NSString *)getShowWeekByDate4:(NSString *)dateStr
{
    ...
    NSString *holiday = [DateUtil getHolidayString:dateStr];
    NSString *weekday = [DateUtil getShowWeekByDate:date];
    NSString *today = [DateUtil getCalendarStrBySimpleDateFormat:date simpleDateFormatType:SIMPLEFORMATTYPE7];
    NSString *todayStr = [DateUtil getDateStrCompareToDay:dateStr];
...
//    NSString *nowDateStr = [DateUtil dateFormateWithDate:nowDate formate:@"yyyyMMdd"];
    ...
    if(holidayStr != nil && ![holidayStr isEqualToString:@""])
    {
        NSString *str = [DateUtil stringOfDate:startDate dateFormat:@"yyyy年"];
//        NSString *weekStr = [DateUtil getShowWeek:dateStr];
...
//        }
    ...
    {
        NSString *str = [DateUtil stringOfDate:startDate dateFormat:@"yyyy年"];
        NSString *weekStr = [DateUtil getShowWeekByDate:startDate];
        retStr = [NSString stringWithFormat:@"%@ %@",str,weekStr];
...
+ (NSString *)getShowStrByDateStr:(NSString *)dateStr
{
    ...
    NSString *retStr = [DateUtil dateFormateWithDate:date formate:SIMPLEFORMATTYPESTRING9];
...
+ (NSDate *)dateFromString:(NSString *)str formate:(NSString *)formate
{
    //    if(dateFormater == nil)
    //    {
    NSDateFormatter *dateFormater = [[DateUtil getCurrentDateFormatter] retain];
    //    }else
    //    {
...
+ (NSDateFormatter *)getCurrentDateFormatter
{
    ...
    [dateFormater setTimeZone:[NSTimeZone timeZoneWithName:[DateUtil displayTimeZone]]];
...
+ (NSDate *)convertStringToDate:(NSString *)dateStr
{
	...
	if ([dateStr length] == 14) {
		...
		NSCalendar *calendar = [DateUtil getCurrentCalendar];
        [calendar setTimeZone:[NSTimeZone timeZoneWithName:[DateUtil displayTimeZone]]];
		NSDateComponents *dateComponents =  [[NSDateComponents alloc] init];
		[dateComponents setYear:year];
		[dateComponents setMonth:month];
		[dateComponents setDay:day];
...
{
    NSDate *firstDate = [DateUtil dateFromString:firstDateStr formate:SIMPLEFORMATTYPESTRING6];
    
    NSDate *secondDate = [DateUtil dateFromString:secondDateStr formate:SIMPLEFORMATTYPESTRING6];
    
    return [DateUtil compareCalendarByLevel:firstDate secondDate:secondDate];
}

#pragma mark 两个日期字串是否相等
+ (BOOL)dateStringEquls:(NSString *)firstDate secondDate:(NSString *)secondDate
{
    NSDate *startDate = [DateUtil dateFromString:firstDate formate:SIMPLEFORMATTYPESTRING6];
    
    NSDate *nowDate  = [DateUtil dateFromString:secondDate formate:SIMPLEFORMATTYPESTRING6];
    
    NSCalendar *chineseClendar = [DateUtil getCurrentCalendar];
...
{    
    NSString *firstStr = [DateUtil dateFormateWithDate:firstDate formate:@"yyyyMMddHHmmss"];
    NSString *secondStr = [DateUtil dateFormateWithDate:secondDate formate:@"yyyyMMddHHmmss"];
...
{
    NSString *firstDateStr = [DateUtil dateFormateWithDate:firstDate formate:@"yyyyMMddHHmmss"];
    NSString *secondDateStr = [DateUtil dateFormateWithDate:secondDate formate:@"yyyyMMddHHmmss"];
...
{
    NSString *firstStr = [DateUtil dateFormateWithDate:firstDate formate:@"yyyyMMdd"];
    NSString *secondStr = [DateUtil dateFormateWithDate:secondDate formate:@"yyyyMMdd"];
...
+ (BOOL)isMiddleDateWithStartDate:(NSDate *)startDate endDate:(NSDate *)endDate middleDate:(NSDate *)currendDate
{
    ...
    NSString *startDateStr = [DateUtil dateFormateWithDate:startDate formate:@"yyyyMMdd"];
    NSInteger start = [startDateStr integerValue];
    
    NSString *endDateStr = [DateUtil dateFormateWithDate:endDate formate:@"yyyyMMdd"];
    NSInteger end = [endDateStr integerValue];
    
    NSString *currendDateStr = [DateUtil dateFormateWithDate:currendDate formate:@"yyyyMMdd"];
...
+ (BOOL)dateCalendarAfterToday:(NSDate *)firstDate
{
    ...
    return [DateUtil firstCalendarAfterSecondCalendar:firstDate secondDate:today];
}
...
+ (BOOL )dateCalendarEqulsToday:(NSDate *)firstDate
{
    ...
    return [DateUtil firstCalendarEquleSecondCalendar:firstDate secondDate:today];
}
...
+ (BOOL )dateCalendarBeforeToday:(NSDate *)firstDate
{
    ...
    return [DateUtil firstCalendarBeforeSecondCalendar:firstDate secondDate:today];
}
...
{
    NSDate *today = [NSDate date];
    NSString *todayStr = [DateUtil dateFormateWithDate:today formate:SIMPLEFORMATTYPESTRING6];
    today = [DateUtil dateFromString:todayStr formate:SIMPLEFORMATTYPESTRING6];
...
+ (BOOL)getDateStringBeforeToday:(NSString *)firstDateStr
{
    ...
    NSDate *firstDate = [DateUtil dateFromString:firstDateStr formate:SIMPLEFORMATTYPESTRING1];
...
+ (NSString *)getDateStringWithYear:(NSInteger )year month:(NSInteger )month day:(NSInteger )day
{
    ...
    return [DateUtil getDateStringWithYear:year month:month day:day hour:0 minute:0 second:0];
}
...
{
    NSDate *date = [DateUtil getDateWithYear:year month:month day:day hour:hour minute:minute second:second];
    
    NSString *retStr = [DateUtil stringOfDate:date dateFormat:SIMPLEFORMATTYPESTRING2];
...
+ (NSDate *)getDateWithYear:(NSInteger)year month:(NSInteger)month day:(NSInteger)day
{
    ...
    NSDate *date = [DateUtil getDateWithYear:year month:month day:day hour:0 minute:0 second:0];
...
+ (NSInteger)compareDaysFromStartDate:(NSDate *)startDate endDate:(NSDate *)endDate
{
    ...
    NSString *sDate = [DateUtil dateFormateWithDate:startDate formate:@"yyyyMMdd"];
    startDate = [DateUtil dateFromString:sDate formate:@"yyyyMMdd"];
    
    NSString *seDate = [DateUtil dateFormateWithDate:endDate formate:@"yyyyMMdd"];
    endDate = [DateUtil dateFromString:seDate formate:@"yyyyMMdd"];
...
+ (NSInteger)compareDaysFromStartDateStr:(NSString *)startDateStr endDateStr:(NSString *)endDateStr
{
    ...
    NSDate *startDate = [DateUtil dateFromString:startDateStr formate:@"yyyyMMdd"];
    NSDate *endDate = [DateUtil dateFromString:endDateStr formate:@"yyyyMMdd"];
...
+ (NSDateComponents*) compareFromDate:(NSString *)startDateStr toDate:(NSString *)endDateStr {
    ...
    NSCalendar* calendar_ = [DateUtil getCurrentCalendar];;
...
+ (NSInteger)compareMinutesFromStartDate:(NSDate *)startDate endDate:(NSDate *)endDate
{
    ...
    NSString *sDate = [DateUtil dateFormateWithDate:startDate formate:@"yyyyMMddHHmm"];
    startDate = [DateUtil dateFromString:sDate formate:@"yyyyMMddHHmm"];
    
    NSString *seDate = [DateUtil dateFormateWithDate:endDate formate:@"yyyyMMddHHmm"];
    endDate = [DateUtil dateFromString:seDate formate:@"yyyyMMddHHmm"];
...
+ (NSDate *)getLastDateOfMonthWithDate:(NSDate *)date
{
    ...
    if(date != nil)
    {
        NSInteger days = [DateUtil getDaysOfMonth:date];
        NSInteger month = [DateUtil getMonth:date];
        NSInteger year = [DateUtil getYear:date];
        
        retDate = [DateUtil getDateWithYear:year month:month day:days];
    }
...
+ (NSDate *)getFirstDateOfMonthWithDate:(NSDate *)date
{
    ...
    if(date != nil)
    {
        ...
        retDate = [DateUtil getDateWithYear:year month:month day:1];
    }
...
//    NSArray *cShuXiang = [NSArray arrayWithObjects:@"鼠",@"牛",@"虎",@"兔",@"龙",@"蛇",@"马",@"羊",@"猴",@"鸡",@"狗",@"猪",nil];
    ...
    //取当前公历年、月、日
    NSDateComponents *components = [[DateUtil getCurrentCalendar] components:NSDayCalendarUnit |NSMonthCalendarUnit | NSYearCalendarUnit fromDate:solarDate];
...
+ (int)yearDays:(int)year
{
    ...
    return sum + [DateUtil leapDays:year];
}
...
+ (eCTStringComparisonResult)compareBetweenDate:(NSDate *)firstDate andDate:(NSDate *)secondDate withFormat:(NSString *)dateFormatString
{
    ...
    NSString *firstDateString = [DateUtil dateFormateWithDate:firstDate formate:dateFormatString];
    NSString *secondDateString = [DateUtil dateFormateWithDate:secondDate formate:dateFormatString];
...
{
    return [DateUtil calculateAgeWithBirthdate:birthdate toNowDate:[NSDate date]];
}
...
+ (NSString *)dateStringFromDate:(NSString *)dateString
{
    ...
    NSDate* date = [DateUtil dateFromString:dateString formate:@"yyyyMMddHHmm"];
    
    NSTimeInterval time = [dateNow timeIntervalSinceDate:date];
    
    NSString* timeDisplay = [DateUtil getCalendarStrBySimpleDateFormat:date simpleDateFormatType:SIMPLEFORMATTYPE4];
    
    if (time > 0) {
        if (time < 60) {
            timeDisplay = @"刚刚";
        }else if(time < 60 * 60 && time >= 60){
            timeDisplay = [NSString stringWithFormat:@"%d分钟前",(NSInteger)time/60];
        }else if(time < 60 * 60 * 24){
            timeDisplay = [NSString stringWithFormat:@"%d小时前",(NSInteger)time/(60 * 60)];
        }
    }
